...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-4">
        <div>
            <h4 class="mb-1">Detail Pelanggan</h4>
            <p class="text-muted mb-0">{{ $customer->code }} - {{ $customer->name }}</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('master.customers.index') }}" class="btn btn-label-secondary">Kembali</a>
            <a href="{{ route('master.customers.edit', $customer) }}" class="btn btn-primary">Edit</a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <dl class="row mb-0">
                ...
                <dt class="col-sm-3">Tipe Pelanggan</dt>
                <dd class="col-sm-9">{{ $types[$customer->customer_type] ?? ucfirst($customer->customer_type) }}</dd>
